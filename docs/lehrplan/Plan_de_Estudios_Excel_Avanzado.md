...
- ¿Has creado alguna vez un formato numérico personalizado?
- ¿Conoces la diferencia entre referencias «relativas» y «absolutas»?
- ¿Tienes experiencia con SVERWEIS y tablas dinámicas sencillas?

Estos conocimientos básicos se presuponen en este plan de estudios. Si no estás seguro de algún concepto,
consulta el plan de estudios **Excel para principiantes** o simplemente pregunta.
...
- **Celda activa incorrecta:** En el formato condicional basado en fórmulas, la fórmula debe referirse a la celda activa de la selección. Si A1 está activa: `=$F1="TX"`, no `=$F5`.
- **Símbolos $ en la fórmula:** Para resaltar filas completas, la fila debe ser relativa: `$F1`, no `$F$1`.
...
- [ ] Crear formatos numéricos personalizados con hasta cuatro secciones
- [ ] Aplicar formato condicional basado en fórmulas para filas completas, vencimientos y duplicados
- [ ] Configurar validación de datos avanzada con fórmulas personalizadas y mensajes de error propios
- [ ] Configurar protección de hoja selectiva con celdas de entrada desbloqueadas y fórmulas ocultas
...
- ¿Ya usas SVERWEIS? ¿Dónde encuentras sus limitaciones?
- ¿Has escrito alguna vez una fórmula que devuelva resultados diferentes según una condición?
- ¿Sabes cómo almacena Excel internamente las fechas (01/01/1900 = 1)?

Este módulo amplía tu repertorio de funciones a nivel profesional.

## 2.1. INDEX + VERGLEICH: la alternativa flexible a SVERWEIS

### Concepto: buscar en cualquier dirección

SVERWEIS solo puede buscar de izquierda a derecha. INDEX + VERGLEICH elimina esta
...
**Búsqueda bidimensional:** `=INDEX(B3:E9; VERGLEICH(B13; B3:B9; 0); VERGLEICH(B14; B3:E3; 0))`
...
**Consejo:** INDEX+VERGLEICH también es más rápido que SVERWEIS en tablas grandes
**Consejo:** En Excel 365/2021, `XVERWEIS` (XLOOKUP) sustituye tanto a SVERWEIS como
a INDEX+VERGLEICH con una sintaxis más sencilla:
...
- **Olvidar el tipo de coincidencia:** Sin `0` como tercer argumento, VERGLEICH busca de forma aproximada, lo que da resultados incorrectos si la lista no está ordenada.
- **Tamaño del rango en INDEX+VERGLEICH:** El rango de INDEX y el rango de VERGLEICH deben tener el mismo número de filas.

**Ejercicio 2.1: Aplicar INDEX + VERGLEICH**

La siguiente tabla de ejercicios **Módulo 2.1 INDEX_VERGLEICH** ya está cargada.

> 1. Busque el precio de un producto con INDEX + VERGLEICH, teniendo en cuenta que la
...
## 2.2. Referencias dinámicas con BEREICH.VERSCHIEBEN

### Concepto: rangos que «respiran»

BEREICH.VERSCHIEBEN (OFFSET) devuelve una referencia desplazada un número determinado
de filas/columnas desde una celda inicial, con altura
y anchura variables.

**Sintaxis:** `=BEREICH.VERSCHIEBEN(referencia; filas; columnas; [altura]; [anchura])`
...
| Aplicación | Fórmula |
|-----------|--------|
| Suma dinámica | `=SUMME(BEREICH.VERSCHIEBEN(B1;0;0;ANZAHL2(B:B);1))` |
| Últimos 3 meses | `=MITTELWERT(BEREICH.VERSCHIEBEN(B1;ANZAHL(B:B)-3;0;3))` |
| Media móvil | En combinación con ZÄHLENWENN para ventanas móviles |

**Importante:** BEREICH.VERSCHIEBEN es una **función volátil**, es decir,
se vuelve a calcular cada vez que se modifica el libro. Si hay muchas fórmulas con BEREICH.VERSCHIEBEN,
el rendimiento puede verse afectado.

**Ejercicio 2.2  Referencias dinámicas**

La siguiente tabla de ejercicios **Módulo 2 2 BEREICH_VERSCHIEBEN** ya está cargada.
...
- **Tasa de interés no ajustada:** El interés anual debe dividirse entre 12 para cuotas mensuales: `RMZ(4,5%/12; 30*12; -250000)`.
- **Signo:** Los pagos que realizas (cuota de préstamo, cuota de ahorro) son negativos. El importe del préstamo es positivo.
...
- [ ] Usar INDEX+VERGLEICH para búsquedas bidireccionales (también derecha→izquierda)
- [ ] Crear nombres de rango dinámicos con BEREICH.VERSCHIEBEN
- [ ] Formular lógica anidada con WENNS, WENNFEHLER y WAHL de forma más elegante que con WENN anidados profundos
- [ ] Aplicar RMZ, NBW, IKV y ZW para cálculos financieros
- [ ] Escribir fórmulas matriciales (clásicas con Ctrl+Mayús+Enter y dinámicas en Excel 365)
- [ ] Realizar cálculos de fecha y hora con MONATSENDE, ARBEITSTAG y NETTOARBEITSTAGE
...
- ¿Trabajas con varias hojas de cálculo en un mismo libro?
- ¿Has vinculado alguna vez datos de otro archivo de Excel?
- ¿Ya utilizas rangos con nombre (p. ej., =Ventas en lugar de =B2:B50)?

Este módulo aporta estructura a tus libros de trabajo y te ahorra tiempo en la búsqueda de errores.
...
| Nombre dinámico | `Datos = BEREICH.VERSCHIEBEN(Tabla1!$A$1;0;0;ANZAHL2(Tabla1!$A:$A);5)` | Crece automáticamente |
...
> 2. Cree un nombre dinámico `TodosLosDatos` con BEREICH.VERSCHIEBEN,
...
| MAX / MIN |  | `=MAX(Región1:Región5!D20)` |
...
- [ ] Usar nombres definidos para constantes, rangos y fórmulas dinámicas de forma profesional
- [ ] Utilizar referencias 3D (Enero:Diciembre!B2) para cálculos entre hojas
- [ ] Crear y gestionar vínculos externos a otros libros de trabajo
- [ ] Realizar consolidación de datos por posición y categoría con vínculo a los datos de origen
...
- ¿Cómo filtras actualmente los datos con varias condiciones?
- ¿Te imaginas poder usar Excel como una pequeña base de datos?
- ¿Sabes qué hace Ctrl+T con un rango de datos?

Técnicas de bases de datos en Excel, sin SQL y sin Access.
...
| DBANZAHL | ANZAHL con criterios | `=DBANZAHLDER(base de datos; «ID»; criterios)` |
...
- [ ] Aplicar filtros especiales con rangos de criterios multinivel Y/O
- [ ] Usar funciones de base de datos (DBSUMME, DBMITTELWERT, DBANZAHL, DBAUSZUG) para evaluaciones flexibles
- [ ] Crear subtotales multinivel con agrupación y esquema
- [ ] Utilizar tablas de Excel con referencias estructuradas ([@NombreColumna]) de forma profesional
...
- ¿Creas tablas dinámicas con regularidad? Si es así: ¿qué te falta?
- ¿Has agrupado alguna vez datos en una tabla dinámica por meses?
- ¿Se pueden filtrar varias tablas dinámicas simultáneamente?

Las técnicas avanzadas de tablas dinámicas convierten simples resúmenes en paneles interactivos.
...
- **Los campos calculados trabajan con valores agregados**, no con datos sin procesar. `Beneficio/Ventas` divide la suma de beneficios entre la suma de ventas, no fila por fila.
- **La agrupación solo es posible en campos de fecha:** Los campos numéricos no se pueden agrupar, solo las fechas.
...
- [ ] Crear tablas dinámicas complejas con varios campos en filas, columnas y valores
- [ ] Aplicar agrupaciones de fechas y campos calculados (p. ej., margen) en tablas dinámicas
- [ ] Usar segmentaciones y escalas de tiempo para paneles interactivos y vincularlas con varias tablas dinámicas
- [ ] Crear gráficos dinámicos que se actualizan automáticamente con la tabla dinámica
- [ ] Comprender las posibilidades de Power Pivot y DAX para modelos de datos grandes
...
- ¿Has «calculado hacia atrás» alguna vez (del resultado deseado a la entrada necesaria)?
- ¿Cómo modelas actualmente diferentes escenarios de futuro?
- ¿Sabes cómo encontrar valores óptimos bajo restricciones?

El análisis condicional es la disciplina suprema del análisis en Excel: aquí aprendes todas las herramientas.
...
- **Solver no activado:** Solver debe activarse primero en Archivo → Opciones → Complementos.
- **Restricciones poco realistas:** Si las restricciones son demasiado estrictas, Solver no encuentra solución. Empieza con límites amplios.
- **Integridad:** Si necesitas números enteros (p. ej., cantidad de productos), añade una restricción de integridad.
...
- [ ] Usar la búsqueda de objetivo para cálculos inversos
- [ ] Crear tablas de datos unidimensionales y bidimensionales para análisis de escenarios
- [ ] Utilizar el administrador de escenarios para previsiones optimistas/pesimistas/neutras
- [ ] Configurar Solver para problemas de optimización con múltiples restricciones
- [ ] Usar minigráficos y líneas de tendencia con R² para análisis de tendencias en miniatura
...
- ¿Qué tipos de gráficos usas habitualmente?
- ¿Has mostrado alguna vez dos escalas de datos diferentes (p. ej., € y %) en un mismo gráfico?
- En tu opinión, ¿qué hace que un panel de control sea bueno?

Visualización a nivel profesional: gráficos que convencen a los responsables de la toma de decisiones.
...
- [ ] Crear gráficos combinados con eje secundario para diferentes escalas
- [ ] Construir gráficos de cascada para efectos acumulados (p. ej., cuenta de pérdidas y ganancias)
- [ ] Diseñar un panel de control profesional con varios gráficos, segmentaciones y minigráficos
- [ ] Aplicar principios de diseño de paneles (máx. 4-6 elementos, colores uniformes, cuadrícula)
...
- ¿Hay tareas recurrentes en tu día a día con Excel (siempre el mismo formato, la misma importación)?
- ¿Cuánto tiempo dedicas semanalmente a estas tareas rutinarias?
- ¿Has oído alguna vez el término «macro» y no sabías qué era?

La automatización empieza con un clic en «Grabar»: no necesitas ser programador.
...
- [ ] Grabar macros de forma absoluta y relativa
- [ ] Ejecutar macros mediante botones, combinaciones de teclas y el cuadro de diálogo de macros
- [ ] Abrir el editor de VBA (Alt+F11) y leer y comprender el código grabado
- [ ] Realizar modificaciones sencillas de código (p. ej., colores) en el editor de VBA
...
- Has grabado macros en el Módulo 8. ¿Te gustaría entender qué significa el código?
- ¿Te imaginas que una macro reaccione automáticamente al modificar una celda?
- ¿Te gustaría tener tus propias funciones de Excel que no existen por defecto?

De la grabación a la programación: el siguiente paso natural.
...
- [ ] Declarar variables con tipos de datos correctos (Integer, Double, String, Range)
- [ ] Escribir condiciones If-Then-Else y bucles For en VBA
- [ ] Programar eventos Worksheet_Change y Workbook_Open para reacciones automáticas
- [ ] Escribir funciones definidas por el usuario (UDF) y usarlas en fórmulas de Excel
- [ ] Aplicar buenas prácticas (Option Explicit, nombres descriptivos, comentarios, manejo de errores)
...
- ¿Compartes archivos de Excel regularmente con compañeros o clientes?
- ¿Cómo te aseguras de que el destinatario use el archivo como está previsto?
- ¿Qué combinaciones de teclas usas a diario y cuáles podrías aprender todavía?

La profesionalidad no solo se refleja en las fórmulas, sino también en la colaboración y la eficiencia.
...
- [ ] Crear plantillas profesionales de Excel (.xltx) con celdas de fórmula protegidas y listas desplegables
- [ ] Preparar libros de trabajo para la colaboración (comentarios, exportación a PDF, protección de hoja)
- [ ] Comprender las posibilidades de Power Query para la importación y transformación de datos
- [ ] Dominar las combinaciones de teclas avanzadas más importantes (Ctrl+[, F9, Alt+=)
