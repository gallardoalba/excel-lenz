...
router.post('/:id/submit', authMiddleware, (req: Request, res: Response) => {
  ...
  if (solution.data && data) {
    ...
    for (const taskCol of taskCols) {
      ...
      for (let row = 0; row < maxRow; row++) {
        ...
        // Flexible comparison: numbers, strings, and null/undefined
        ...
        if (userNum !== null && solNum !== null && !isNaN(userNum) && !isNaN(solNum)) {
          ...
          // Normalize text: trim whitespace, lowercase for case-insensitive comparison
  ...
  // Build cell-level feedback
  const details: { row: number; col: number; expected: any; got: any }[] = [];
  if (solution.data && data && score < 100) {
    const taskCols = JSON.parse(exercise.template_data || '{}').taskCols || [];
    for (const taskCol of taskCols) {
      for (let row = 0; row < Math.min(solution.data.length, data.length); row++) {
        const userVal = data[row]?.[taskCol];
        const solVal = solution.data[row]?.[taskCol];
        const userNum = userVal !== null && userVal !== undefined ? Number(userVal) : null;
        const solNum = solVal !== null && solVal !== undefined ? Number(solVal) : null;
        const isCorrect = (userNum !== null && solNum !== null && Math.abs(userNum - solNum) < 0.01) ||
                          (String(userVal ?? '') === String(solVal ?? ''));
        if (!isCorrect) {
          details.push({ row, col: taskCol, expected: solVal, got: userVal });
        }
      }
    }
  }
